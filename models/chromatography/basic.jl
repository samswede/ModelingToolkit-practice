...
function test(L::Float64, T::Float64)
    ...
    ic_bc = [
        ...
        c(0.0, x) ~ 0.0,
        # boundary conditions
        ∂x(c(t, 0)) ~ u * (c(t, 0) - c0) / D, #AH! c(t, x) should be c(t, 0) (I think)
        ∂x(c(t, L)) ~ 0.0 # This is a problem for some reason.
        # Upper boundary condition Differential(x)(c(t, 20.0)) ~ 0.0 on time variable is not supported, please use a change of variables `t => -τ` to make this an initial condition.
    ]
